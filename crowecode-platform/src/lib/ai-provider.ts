...
// Export individual functions for AI endpoints
export const generateCode = async (prompt: string, options?: any) => {
  return await aiProviderManager.executeWithFallback({ prompt, ...options }, 'code_generation');
};

export const generateCompletion = async (prompt: string, options?: any) => {
  return await aiProviderManager.executeWithFallback({ prompt, ...options }, 'code_completion');
};
...
export const anthropic = {
  ...
  generateContent: async (prompt: string) => {
    return aiProviderManager.executeWithFallback({ prompt }, 'code_generation');
  }
};