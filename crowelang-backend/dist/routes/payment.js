...
router.post('/create-checkout-session', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.logger.info(`Checkout session created for user ${userId}, plan: ${plan}`);
        res.json({
...
router.post('/create-portal-session', auth_1.authenticate, async (req, res) => {
    try {
...
router.get('/subscription', auth_1.authenticate, async (req, res) => {
    try {
        const user = await User_1.User.findById(req.user.id).populate('licenses');
        if (!user) {
        ...
        const activeLicense = licenses.find(license => license.status === 'active' && license.expiresAt > new Date());
        res.json({
...
// Apply promo code
router.post('/apply-promo-code', auth_1.authenticate, async (req, res) => {
    try {
        const { code } = req.body;
        ...
        logger_1.logger.error('Error applying promo code:', error);
