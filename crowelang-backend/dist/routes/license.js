...
router.post('/validate', async (req, res) => {
    try {
        ...
        const license = await License_1.License.findOne({ licenseKey }).populate('userId', 'email');
        if (!license) {
        ...
        logger_1.logger.error('Error validating license:', error);
        res.status(500).json({
            valid: false,
...
router.post('/track-usage', async (req, res) => {
    try {
        ...
        const license = await License_1.License.findOne({ licenseKey });
        if (!license) {
        ...
        logger_1.logger.info(`Usage tracked for license ${licenseKey}: ${type}`, {
...
router.get('/info', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching license info:', error);
...
router.post('/activate', auth_1.authenticate, async (req, res) => {
    try {
        ...
        license.metadata.hardwareFingerprint.push(hardwareFingerprint);
        await license.save();
        logger_1.logger.info(`Device activated for license ${licenseKey}`, {
            licenseId: license.id,
            plan: license.plan,
        ...
        logger_1.logger.error('Error activating license:', error);
...
router.post('/deactivate', auth_1.authenticate, async (req, res) => {
    try {
        ...
        await license.save();
        logger_1.logger.info(`Device deactivated for license ${licenseKey}`, {
            licenseId: license.id,
        ...
        logger_1.logger.error('Error deactivating license:', error);
