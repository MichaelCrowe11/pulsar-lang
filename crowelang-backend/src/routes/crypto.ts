...
import { Charge, resources, Webhook } from 'coinbase-commerce-node';
...
router.post('/create-charge', authenticate, async (req, res) => {
  try {
    ...
    if (!CRYPTO_PRICING_PLANS[plan as keyof typeof CRYPTO_PRICING_PLANS]) {
      return res.status(400).json({ error: 'Invalid pricing plan' });
    }

    const pricingPlan = CRYPTO_PRICING_PLANS[plan as keyof typeof CRYPTO_PRICING_PLANS];
    ...
    logger.info(`Crypto charge created for user ${user.id}, plan: ${plan}`, {
      chargeId: charge.id,
    ...
    logger.error('Error creating crypto charge:', error);
...
router.get('/charge/:chargeId', authenticate, async (req, res) => {
  try {
    ...
    logger.error('Error retrieving crypto charge:', error);
...
router.get('/charges', authenticate, async (req, res) => {
  try {
    ...
    const charges = await Charge.list({ limit: 100 });
    
    const userCharges = charges.data.filter(charge => 
      charge.metadata?.userId === user.id
    );
    ...
    logger.error('Error listing crypto charges:', error);
...
router.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    ...
    logger.info(`Crypto webhook received: ${event.type}`, {
      eventId: event.id,
    ...
    logger.error('Crypto webhook verification failed:', error);
...
async function handleChargeConfirmed(charge: any) {
  try {
    ...
    if (!userId || !plan) {
      logger.error('Missing metadata in crypto charge', { chargeId: charge.id });
      return;
    }
    ...
    if (existingLicense) {
      logger.info(`License already exists for crypto charge ${charge.id}`);
      return;
    }
    ...
    const licenseData = {
      userId,
      plan: plan as 'personal' | 'professional' | 'team',
    ...
    logger.info(`License created for crypto payment`, {
    ...
    logger.error('Error handling confirmed crypto charge:', error);
  }
}

// Handle failed crypto payment
async function handleChargeFailed(charge: any) {
  logger.warn(`Crypto charge failed: ${charge.id}`, {
    reason: charge.timeline?.find((t: any) => t.status === 'FAILED')?.context
  });
}

// Handle delayed crypto payment
async function handleChargeDelayed(charge: any) {
  logger.info(`Crypto charge delayed: ${charge.id}`, {
    delayReason: charge.timeline?.find((t: any) => t.status === 'DELAYED')?.context
  });
}

// Handle pending crypto payment
async function handleChargePending(charge: any) {
  logger.info(`Crypto charge pending: ${charge.id}`);
}

// Handle resolved crypto payment (after being delayed)
async function handleChargeResolved(charge: any) {
  logger.info(`Crypto charge resolved: ${charge.id}`);
  // Treat as confirmed if payment was successful
  if (charge.timeline?.some((t: any) => t.status === 'COMPLETED')) {
